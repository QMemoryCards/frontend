...
describe('IT-F-01.1 Создание колоды с валидацией и отображением в списке', () => {
  it('показывает ошибку валидации, создает колоду и отображает ее в списке', async () => {
    ...
    const reopenedDialog = await screen.findByRole('dialog', { name: /создать новую колоду/i });
    expect(within(reopenedDialog).getByPlaceholderText('Введите название колоды')).toHaveValue('');
    expect(
      within(reopenedDialog).getByPlaceholderText('Добавьте описание колоды (необязательно)')
    ).toHaveValue('');
    await user.click(within(reopenedDialog).getByRole('button', { name: 'Отмена' }));
